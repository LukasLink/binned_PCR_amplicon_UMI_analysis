...
subsample_controls  <- TRUE     # bolean keeps 20% of control sgRNAs for reference.
...
compare_subsamples <- function(df_list, percentage_list, FDR_threshold = 0.05, top_N = NULL) {
  ...
  process_scenario <- function(df_list, percentage_list, FDR_threshold, significance_filter = NULL, top_N = NULL) {
    ...
    for (i in seq_along(df_list)) {
      ...
      Subsample_percentage <- c(Subsample_percentage, percentage_list[i])
    ...
    final_results <- data.frame(
      Subsample_percentage = unlist(Subsample_percentage),
  ...
  all_hits_df <- process_scenario(df_list, percentage_list, FDR_threshold)
  print(all_hits_df)
  sig_up_hits_df <- process_scenario(df_list, percentage_list, FDR_threshold, significance_filter = 1)
  sig_down_hits_df <- process_scenario(df_list, percentage_list, FDR_threshold, significance_filter = -1)
  ...
  plot_overlap <- function(df, title) {
    ...
    if (!all(c("Subsample_percentage", "Overlap", "Overlap_percentage", "Total_Hits") %in% colnames(df))) {
      stop("The dataframe must contain 'Subsample_percentage', 'Overlap', 'Total_Hits' and 'Overlap_percentage' columns.")
    }
    
    # Create the plot
    p <- ggplot(df, aes(x = Subsample_percentage)) +
      geom_line(aes(y = Overlap_percentage), size = 1) + 
  ...
  plot_total_hits <- function(df, title) {
    # Ensure the input dataframe contains the necessary columns
    if (!all(c("Subsample_percentage", "Overlap", "Overlap_percentage", "Total_Hits") %in% colnames(df))) {
      stop("The dataframe must contain 'Subsample_percentage', 'Overlap', 'Total_Hits' and 'Overlap_percentage' columns.")
    }
  ...
  for (scenario_name in names(results_list)) {
    p <- plot_overlap(results_list[[scenario_name]], scenario_name)
    print(p) 
    q <- plot_total_hits(results_list[[scenario_name]], scenario_name)
    print(q)
  }
  return(results_list)
}

automate_comparison <- function(prefixstring, FDR_threshold = 0.05) {
  ...
  rds_files <- list.files(rds_output_folder, pattern = paste0(".*", prefixstring, "_[0-9]+\\.rds$"), full.names = TRUE)
  
  # Extract the numbers from the file names (e.g., "lukas_reads_control_median_RI_25.rds" -> 25)
  numbers <- as.numeric(gsub(paste0(".*", prefixstring, "_([0-9]+)\\.rds$"), "\\1", rds_files))
  
  # Generate the percentage list (starting from 100 and then descending from the available numbers)
  percentage_list <- sort(numbers, decreasing = TRUE)
  
  # Generate Hits_sub_x using add_info_wrapper for each percentage in percentage_list
  df_list <- lapply(percentage_list, function(perc) {
    add_info_wrapper(paste0(prefixstring, "_", perc))
  })
  
  # Call compare_subsamples with the generated df_list and percentage_list
  result <- compare_subsamples(df_list = c(list(Hits_current_settings), df_list),
                                percentage_list = c(100, percentage_list),
...
``` {r compare_subsamples, eval=False}
Hits_sub_25 <- add_info_wrapper("lukas_reads_control_median_RI_25")
Hits_sub_50 <- add_info_wrapper("lukas_reads_control_median_RI_50")
Hits_sub_75 <- add_info_wrapper("lukas_reads_control_median_RI_75")

target_df <- Hits_sub_25 %>% filter(FDR <= 0.05)
reference_df <- Hits_current_settings %>% filter(FDR <= 0.05)
reference_entrez <- reference_df$entrez
target_entrez <- target_df$entrez
overlap_count <- sum(target_entrez %in% reference_entrez)
print(overlap_count)
print(setdiff(reference_entrez,target_entrez))

test <- automate_comparison("lukas_reads_control_median_RI", FDR_threshold = 0.005)
test <- compare_subsamples(df_list = list(Hits_current_settings,Hits_sub_75, Hits_sub_50, Hits_sub_25),
                   percentage_list = c(100,75,50,25),
                   FDR_threshold = 0.01,
                   top_N = NULL)

compare_100_75 <- compare_Hits(Hits_current_settings, Hits_sub_75, top_N=341)
comp_overlap_100_75 <- compare_100_75$Overlap
comp_dropout_100_75 <- compare_100_75$Dropouts
comp_statistics_100_75 <- compare_100_75$Statistics
compare_100_50 <- compare_Hits(Hits_current_settings, Hits_sub_50, top_N=341)
comp_overlap_100_50 <- compare_100_50$Overlap
comp_dropout_100_50 <- compare_100_50$Dropouts
comp_statistics_100_50 <- compare_100_50$Statistics
compare_100_25 <- compare_Hits(Hits_current_settings, Hits_sub_25, top_N=341)
comp_overlap_100_25 <- compare_100_25$Overlap
comp_dropout_100_25 <- compare_100_25$Dropouts
comp_statistics_100_25 <- compare_100_25$Statistics
compare_statistics <- comp_statistics_100_75 %>% 
  rename(c_100_vs_75 = Value) %>% 
  mutate(c_100_vs_50 = comp_statistics_100_50$Value,
         c_100_vs_25 = comp_statistics_100_25$Value)
...
```{r Roche_compare_functions, eval=TRUE}
Hits_current_settings <- add_info_wrapper(file_info_suffix)
if (length(include_controls_list) > 0) {
  for (control_gene in include_controls_list) {
    # here we remove everything after the last _, so stuff like AAVS1_9 and
