...
output_folder   <- "/g/steinmetz/link/Amplicon_barcode_analysis/Liangfu_iBeer_2/Texas"
...
subsample_controls  <- FALSE     # bolean keeps 20% of control sgRNAs for reference.
...
make_compare_df <- function(df_list, name_list, FDR_threshold, significance_filter = NULL, top_N = NULL) {
  ...
  if (length(df_list) != length(name_list)){
    stop("ERROR: in make_compare_df -> length of df_list must be equal to lenght of name_list")
  }
...
move_subsamples <- function(subsample_master_dir,
  ...
  for (sd in subsample_dirs) {
    ...
    for (file in files) {
      ...
      new_base <- sub("\\.rds$", paste0("_subsample", X, ".rds"), base)
...
automate_subsample_comparison <- function(prefixstring = get("file_info_suffix",
                                                             envir = .GlobalEnv),
                                          FDR_threshold = 0.05) {
  ...
  percentage_list <- unique(percentage_list)
  ...
  df_list <- lapply(percentage_list, function(perc) {
    add_info_wrapper(paste0(prefixstring, "_subsample", perc))
  })
  ...
  result <- compare_df_list(df_list = c(list(Hits_current_settings), df_list),
                            name_list = c(100, percentage_list),
                            FDR_threshold = FDR_threshold,
...
find_Hits_and_FDR_appearing_in_X_replicates <- function(df_list, name_list, FDR_threshold = 0.05, X = NULL){
  if (is.null(X)){
    X <- length(df_list)
  }
...
automate_calc_replicate_comparison <- function(file_info_suffix = get("file_info_suffix", envir = .GlobalEnv),
  ...
  rep_name_long <- unique(rep_name_long)
...
calc_rep_analysis <- automate_calc_replicate_comparison(FDR_threshold = 0.01)

excel_file_path <- sub("\\.rds$", ".xlsx", paste0(output_folder, "/", "Hits_in_all_runs_", file_suffix))
write_xlsx(calc_rep_analysis$Hits_in_X_df, excel_file_path)
```
...
if (FALSE){
  ...
  move_subsamples("/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep_PA_subsample")
}
