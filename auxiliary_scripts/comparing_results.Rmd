...
add_info_to_gene_stats <- function(maude_guide_stats, maude_gene_stats){
  ...
  essential_df <- read.delim(
    "/g/steinmetz/link/Amplicon_barcode_analysis/RAW/essential_genes.txt",
    header = FALSE,
    sep = "\t",
    stringsAsFactors = FALSE
  ) %>%
    setNames(c("symbol", "entrez")) %>%   # naming as you requested
    mutate(entrez = sub("^HGNC:", "", entrez))  # remove "HGNC:"

  export_df <- export_df %>%
  mutate(
    essential = if_else(
      as.character(entrez) %in% as.character(essential_df$entrez),
      "Yes",
      "No",
      missing = "No"
    )
...
high_confidence_hits <- automate_calc_replicate_comparison(FDR_threshold = 0.2,
                                                           hits_in_X_reps = 16,
                                                           correlation_heatmap = FALSE,
                                                           venn_diagram = FALSE)
inclusive_hits <- automate_calc_replicate_comparison(FDR_threshold = 0.2,
                                                     hits_in_X_reps = 10,
                                                     correlation_heatmap = FALSE,
                                                     venn_diagram = FALSE)
...
write_xlsx(inclusive_hits$Hits_in_X_df, excel_file_path)
```
