...
output_folder   <- "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep_DCA"
...
# Point to the data subdirectory with my pre-processed info files
rmd_path <- Sys.getenv("SOURCE_RMD", unset = NA_character_)

rmd_dir <- if (!is.na(rmd_path) && nzchar(rmd_path)) {
  dirname(normalizePath(rmd_path))
} else {
  # fallback: script dir (when running a normal .R script)
  tryCatch(dirname(normalizePath(sys.frame(1)$ofile)),
           error = function(e) getwd())
}

data_dir <- file.path(rmd_dir, "data")
data_dir <- sub("auxiliary_scripts/", "", data_dir)
