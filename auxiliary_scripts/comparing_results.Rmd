...
################################################################################
############# LOAD COMPARISION SPECIFIC LIBS ###################################
library(VennDiagram)
library(grid)
library(gridExtra)
...
all_conjugates_overlap_df <- read.csv2(
  "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/Explorative_Hits_Overlap_between_conjugates.csv",
  header = TRUE
) 
...
saveRDS(all_conjugates_overlap_classified, "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/all_conjugates_overlap_classified.rds")
saveRDS(validation_hits, "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/validation_hits.rds")

```

``` {r barplot_Z-score_distribution, eval=False}
...
pathways_hit_enriched_by_cat <- run_pathway_by_category(
  df = all_conjugates_overlap_df,
  file_info_suffix = file_info_suffix,
  method = "enrichGO"
)
saveRDS(pathways_hit_enriched_by_cat, "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/pathways_enriched_in_hits.rds")
pathways_all <- pathways_hit_enriched_by_cat$shared_all_three
pathways_DCA_GAL <- pathways_hit_enriched_by_cat$shared_DCA_GalNAc
pathways_DCA_PA <- pathways_hit_enriched_by_cat$shared_DCA_PA
pathways_PA_GAL <- pathways_hit_enriched_by_cat$shared_PA_GalNAc
pathways_PA <- pathways_hit_enriched_by_cat$unique_PA
pathways_DCA <- pathways_hit_enriched_by_cat$unique_DCA
pathways_GAL <- pathways_hit_enriched_by_cat$unique_GalNAc

all_conjugates_df <- calculate_category_based_Z_score(all_conjugates_overlap_df)

all_conjugates_pos_df <- all_conjugates_df %>% filter(significanceZ > 0)
all_conjugates_neg_df <- all_conjugates_df %>% filter(significanceZ < 0)

# positive hits
pathways_hit_enriched_by_cat_pos <- run_pathway_by_category(
  df = all_conjugates_pos_df,
  file_info_suffix = file_info_suffix,
  method = "enrichGO"
)

saveRDS(
  pathways_hit_enriched_by_cat_pos,
  "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/pathways_enriched_in_pos_hits.rds"
)

pathways_all_pos     <- pathways_hit_enriched_by_cat_pos$shared_all_three
pathways_DCA_GAL_pos <- pathways_hit_enriched_by_cat_pos$shared_DCA_GalNAc
pathways_DCA_PA_pos  <- pathways_hit_enriched_by_cat_pos$shared_DCA_PA
pathways_PA_GAL_pos  <- pathways_hit_enriched_by_cat_pos$shared_PA_GalNAc
pathways_PA_pos      <- pathways_hit_enriched_by_cat_pos$unique_PA
pathways_DCA_pos     <- pathways_hit_enriched_by_cat_pos$unique_DCA
pathways_GAL_pos     <- pathways_hit_enriched_by_cat_pos$unique_GalNAc
pathways_allhits_pos <- pathways_hit_enriched_by_cat_pos$All_Hits


# negative hits
pathways_hit_enriched_by_cat_neg <- run_pathway_by_category(
  df = all_conjugates_neg_df,
  file_info_suffix = file_info_suffix,
  method = "enrichGO"
)

saveRDS(
  pathways_hit_enriched_by_cat_neg,
  "/g/steinmetz/link/Amplicon_barcode_analysis/HepG2_dual_rep/pathways_enriched_in_neg_hits.rds"
)

pathways_all_neg     <- pathways_hit_enriched_by_cat_neg$shared_all_three
pathways_DCA_GAL_neg <- pathways_hit_enriched_by_cat_neg$shared_DCA_GalNAc
pathways_DCA_PA_neg  <- pathways_hit_enriched_by_cat_neg$shared_DCA_PA
pathways_PA_GAL_neg  <- pathways_hit_enriched_by_cat_neg$shared_PA_GalNAc
pathways_PA_neg      <- pathways_hit_enriched_by_cat_neg$unique_PA
pathways_DCA_neg     <- pathways_hit_enriched_by_cat_neg$unique_DCA
pathways_GAL_neg     <- pathways_hit_enriched_by_cat_neg$unique_GalNAc
pathways_allhits_neg <- pathways_hit_enriched_by_cat_neg$All_Hits
