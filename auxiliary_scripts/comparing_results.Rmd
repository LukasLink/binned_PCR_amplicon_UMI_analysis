...
# output_folder   <- "/g/steinmetz/link/Amplicon_barcode_analysis/Liangfu_iBeer_2/COR"
...
add_info_to_gene_stats <- function(maude_guide_stats, maude_gene_stats){
  ...
  HepG2_tpm_Xie <- readRDS("/g/steinmetz/link/Liangfu_Expression_Heatmaps/rds_files/hepato_df_tpm.rds")
  
  export_df <- export_df %>%
  left_join(
    HepG2_tpm_Xie %>%
      filter(!is.na(entrez_id)) %>%
      select(entrez_id, HepG2_TPM_Xie = HepG2, primary_hepato_TPM_Xie = primary),
    by = c("entrez" = "entrez_id")
  )
  
  # Add Nicos HepG2 RNAseq expression data
  HepG2_tpm_Battisti <- readRDS("/g/steinmetz/link/Amplicon_barcode_analysis/RAW/Nico_HepG2_dual_rep_RNAseq/Nico_HepG2_dual_rep_RNAseq.rds")
    export_df <- export_df %>%
      left_join(
        HepG2_tpm_Battisti %>%
          filter(!is.na(entrez)) %>%
          select(entrez,
                 HepG2_TPM_Battisti = TPM_WT,
                 HepG2_14D9_TPM_Battisti = TPM_dual_rep_14D9,
                 HepG2_14B11_TPM_Battisti = TPM_dual_rep_14B11),
        by = "entrez"
      )
...
high_confidence_hits <- automate_calc_replicate_comparison(FDR_threshold = 0.01, hits_in_X_reps = 16)
inclusive_hits <- automate_calc_replicate_comparison(FDR_threshold = 0.01, hits_in_X_reps = 10)

excel_file_path <- sub("\\.rds$", ".xlsx", paste0(results_output_folder, "/", "Hits_high_confidence_", file_suffix))
write_xlsx(calc_rep_analysis$Hits_in_X_df, excel_file_path)
excel_file_path <- sub("\\.rds$", ".xlsx", paste0(results_output_folder, "/", "Hits_explorative_", file_suffix))
write_xlsx(calc_rep_analysis$Hits_in_X_df, excel_file_path)
```
...
```
``` {r control_distribution_heatmap, eval=TRUE}
library(pheatmap)
library(grid)
