...
#SBATCH -A lsteinme             # profile of the group                # sbatch /home/link/Run_R_markdown_array.sh   # sbatch --dependency=afterok:24467230 /home/link/Run_R_markdown.sh
...
# Define combinations
pipelines=("john" "john" "john")
data_type=("reads" "reads" "reads")
methods=("" "sum" "rep")
extra_suffix=("rf" "" "")
...
R_SCRIPT="/scratch/link/temp/$(basename "$RMD_FILE" .Rmd)_${PIPELINE}_${METHOD}.R"

# Logic to handle different modes
if [ "$MODE" == "script" ]; then
  echo "Running Rmd as plain R script..."
  Rscript -e "knitr::purl('$RMD_FILE', output='$R_SCRIPT', documentation = 0)"
  Rscript "$R_SCRIPT" --pipeline "john" --data_type "$DATA_TYPE" --method "$METHOD" --drop_0s FALSE --recover_input TRUE --extra_suffix "$EXTRA_SUFFIX"
