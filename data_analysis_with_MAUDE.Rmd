...
include_controls_list <- c("EGFP","AAVS1")
...
extra_suffix  <- "rf"   # custom sufix, special behaviour when set to "rf"
...
generate_all_violin_plots_and_summaries(count_df_long, y_limit = 12000, non_targeting = TRUE)
...
Hits_sum <- add_info_wrapper("lukas_umis_sum_control_median_RI")
...
Hits_current_settings <- add_info_wrapper(file_info_suffix)
if (length(include_controls_list) > 0) {
  # Loop over each control gene
  for (control_gene in include_controls_list) {
    # Update the 'symbol' column for the current control gene
    Hits_current_settings$symbol[Hits_current_settings$entrez == control_gene] <- control_gene
  }
  ...
  if (mark_cntrl && "symbol" %in% names(Hits_df)) {
    p <- p + geom_point(
      ...
      annotate("rect",
               ...
               ymin = min(Hits_df$significanceZ) + 0.2 - max(Hits_df$significanceZ)/40,
               ymax = min(Hits_df$significanceZ) + 0.7 + max(Hits_df$significanceZ)/40,
...
plot_significance_by_rank(Hits_current_settings_no_EGFP,
                          mark_special = "AHSA1",
                          mark_N_top_hits = 3,
                          box_padding = 0.6)
