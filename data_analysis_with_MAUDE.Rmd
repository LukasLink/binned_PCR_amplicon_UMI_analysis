...
#-------------------------------------------------------------------------------
# Small helper for optional YAML fields
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Previous behavior:
# if FALSE, remove use_only_these_controls_list if it exists.
#
# New behavior:
...
#-------------------------------------------------------------------------------
# This is useful for reproducibility, especially when iterating over conditions.
...
if (!is.null(user_opts$output_folder) && nzchar(user_opts$output_folder)) {
  ...
  used_config_name <- if (nzchar(user_opts$extra_suffix)) {
    paste0("used_config_", user_opts$extra_suffix, ".yaml")
  } else {
    "used_config.yaml"
  }
...
if (read_counting == "bcwithqc"){
  ...
  if (read_counting == "align_UMI_tools"){
    ...
    if (read_counting == "bcwithqc_rf"){
      ...
      stop(read_counting, " is not a valid read_counting value. Exiting. read_counting must be bcwithqc, bcwithqc_rf or align_UMI_tools.")
    }
  }  
}
sink()
