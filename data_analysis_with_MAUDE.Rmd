...
process_folder_files <- function(folder_path,
  ...
  files <- list.files(folder_path, pattern = "^[ILU]_L[1-6]_\\d+_dedup_idxstats\\.txt$", full.names = TRUE)
  ...
  combined_df <- bind_rows(lapply(files, function(file) {
    ...
    matches <- str_match(file_name, "^([ILU])_L([1-6])_(\\d+)_dedup_idxstats\\.txt$")
    ...
    df <- read_file_to_df(file_name = file_name, folder_path = folder_path, threshold_df = threshold_df)
...
complete_sgrna_df <- function(df, merged_sgRNA_df, sublib) {
  expected_ids <- merged_sgRNA_df %>%
    filter(sublib == !!sublib) %>%
    pull(sgrna_id)
...
process_john_rf_data <- function(parent_dir,
  ...
  for (file in file_paths){
    ...
    # Extract condition (X), sublib (L#), and sample (#) using regex
    pattern <- paste0("^([ILU])_L([1-6])_([1-9])", suffix_to_rm)
    ...
    # Safely read data
    df <- tryCatch({
      df <- read_john_rf_data(file, sgRNA_df)
      df <- complete_sgrna_df(df, merged_sgRNA_df, sublib)
      ...
      warning(paste("Skipping due to error:", name, "->", e$message))
      return(NULL)
    })
...
process_john_data <- function(parent_dir,
  ...
  for (dir_path in subdirs) {
    ...
    # Safely read data
    df <- tryCatch({
      df <- read_john_data(matrix_dir, sgRNA_df)
      df <- complete_sgrna_df(df, merged_sgRNA_df, sublib)
      ...
      warning(paste("Skipping due to error:", folder_name, "->", e$message))
      return(NULL)
    })
...
normalize_count_df_long <- function(count_df_long, norm_method = "control_median"){
  ...
  norm_fac <- count_df_long %>%
    filter(group_category %in% c("targeting_control", "non_targeting_control")) %>%
...
generate_all_violin_plots_and_summaries(count_df_long, y_limit = 60, non_targeting = TRUE)
generate_all_violin_plots_and_summaries(count_df_long, norm_method = "control_median", y_limit = 30, non_targeting = TRUE) 
...
plot_maude_qc <- function(maude_counts_df, print = TRUE, add_0s = FALSE) {
  ...
  p1 <- ggplot(na_summary_overall, aes(x = condition, y = na_count, fill = condition)) +
    geom_col() +
    labs(title = "Overall NA Counts per Condition", y = "Number of NAs", x = "Condition") +
    theme_bw()
  ...
  p2 <- ggplot(na_summary_by_exp, aes(x = exp, y = na_count, fill = condition)) +
    ...
    labs(title = "NA Counts per Condition by Experiment", x = "Experiment", y = "Number of NAs") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
sgRNA_counts <- count_df_long %>%
  ...
  mutate(group = interaction(condition, exp, drop = TRUE))  # for x-axis

# 2. Plot
p <- ggplot(sgRNA_counts, aes(x = group, y = num_sgRNAs, fill = condition)) +
  geom_col() +
  labs(
    title = "Number of sgRNA Entries per (Condition, Exp)",
    x = "Condition x Experiment",
    y = "sgRNA Count"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
